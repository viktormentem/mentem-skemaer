#!/usr/bin/env bash
# Runner for klient-inbox Fase 2 VERA (udgaaende-konvolut tovejs).
# (1) custody-level dekrypt-roundtrip (ingen worker) -> (2) integration-drive mod LOKAL `wrangler dev`.
# Dev/test-only. NUL rigtig klient-data / prod.
set -euo pipefail
...
echo "== Fase2-VERA: worker=$WORKER_DIR base=$BASE =="

echo "-- (1) custody-level dekrypt-roundtrip (ingen worker) --"
cd "$SKEMAER_DIR"
node test/inbox-fase2-dekrypt.mjs
...
npx wrangler d1 migrations apply "$D1_NAME" --local >/tmp/mycel-fase2-vera-migrate.log 2>&1 || {
  echo "FEJL: d1 migrate; se /tmp/mycel-fase2-vera-migrate.log"; tail -20 /tmp/mycel-fase2-vera-migrate.log; exit 2; }
...
set -m
npx wrangler dev --port "$PORT" --ip 127.0.0.1 >/tmp/mycel-fase2-vera-dev.log 2>&1 &
DEV_PID=$!
set +m
...
[ "$ok" = 1 ] || { echo "FEJL: worker svarede ikke paa /health inden timeout; se /tmp/mycel-fase2-vera-dev.log"; tail -30 /tmp/mycel-fase2-vera-dev.log; exit 2; }

echo "-- (2) integration-drive node-VERA --"
cd "$SKEMAER_DIR"
node test/inbox-fase2-drive.mjs
RC=$?
exit $RC
